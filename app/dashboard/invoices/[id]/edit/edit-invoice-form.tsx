...
function parseStoredItems(raw: unknown): LineItem[] {
  if (!Array.isArray(raw) || raw.length === 0) {
    return [{ id: nextId++, description: "", quantity: 1, unit_price: 0 }];
  }
  return (raw as StoredItem[]).map((it) => ({
    id: nextId++,
...
export function EditInvoiceForm({
  ...
  const [recipientType, setRecipientType] = useState<"client" | "company">(
    invoice.company_id ? "company" : "client"
  );
  const [vatRate, setVatRate] = useState<number>(invoice.vat ?? 0);
  const [items, setItems] = useState<LineItem[]>(() => parseStoredItems(invoice.items));
  ...
  const invoiceNumber = `FAC-${new Date(invoice.created_at).getFullYear()}-${invoice.id.slice(0, 8).toUpperCase()}`;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1 font-mono">{invoiceNumber}</p>
      </div>
